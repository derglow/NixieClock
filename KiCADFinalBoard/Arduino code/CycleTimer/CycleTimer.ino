...
uint16_t previousmilliSTime;
uint16_t newmilliSTime;
...
delay(1000);
newmilliSTime=millis();
timeDifference=newmilliSTime-previousmilliSTime;
sendSixDigits(nthdig(6,timeDifference),nthdig(5,timeDifference),nthdig(4,timeDifference),nthdig(3,timeDifference),nthdig(2,timeDifference),nthdig(1,timeDifference));
}
  ...
  if(processReceivedByte==1){ //Check serial flag
    if(receivedByte==36){ //Check for $
      ...
      if((receivedByte == 0x0D) & (serialReceiveBuffer[3] == 0x47) & (serialReceiveBuffer[4] == 0x47) & (serialReceiveBuffer[5] == 0x41)){//if received byte is carriage return and GPGGA is found, save the time
        ...
        newmilliSTime=millis();
        timeDifference=newmilliSTime-previousmilliSTime;
        sendSixDigits(10,10,10,nthdig(2,timeDifference),nthdig(1,timeDifference),nthdig(0,timeDifference));
        previousmilliSTime=newmilliSTime;
      }
      processReceivedByte=0; //clear the flag
    }
  }
